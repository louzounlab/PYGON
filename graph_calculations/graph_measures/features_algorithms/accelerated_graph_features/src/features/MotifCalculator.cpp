...
vector<vector<unsigned int>*>* MotifCalculator::Calculate() {
    /*
    NOTE: Unlike the GPU function, here we keep a variable named "visited_vertices". It might increase the memory usage
    to O(N^2), however we assume that the CPU is not troubled by this (for the GPU avoiding using it was crucial).
    */
	if (this->level == 3) {
...
MotifCalculator::~MotifCalculator() {
	...
	//the index in which we remove the node from the graph. Basically, from this index on the node doesn't exist.
	delete removalIndex;
